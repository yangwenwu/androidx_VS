...
public class DeviceConfig {
	...
	public static void initScreenSize(Context context) {
		if (w == 0 || h == 0) {
			LogFactory.createLog().e("手机分辨率w：" + w + "  h:" + h);
	...
	public static void reinstallScreenSize(Context context) {
		...
		w = size.widthPixels;
		h = size.heightPixels;
		LogFactory.createLog().e("reinstallScreenSize 手机分辨率w：" + w + "  h:" + h);
	}
	...
	public static void resetNetStatus(Context context) {
		...
		LogFactory.createLog().e("网络发生变化，重新获取：" + net);
	}
	...
	public static void initDeviceData(Context context) {
		...
		if (StringUtils.isEmpty(net)) {
			...
			LogFactory.createLog().e("获取网络：" + net);
		}
		...
		if (StringUtils.isEmpty(country)) {
			...
			LogFactory.createLog().e("deviceinfo, country:" + country);
		}
	}
