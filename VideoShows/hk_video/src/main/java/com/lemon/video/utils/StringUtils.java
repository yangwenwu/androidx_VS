...
public class StringUtils {
	...
	public static boolean checkPwdCharacter(String pwd) {
		...
		try {
			...
			LogFactory.createLog().e("lhc,checkPwdCharacter m.matches:" + flag);
		} catch (Exception e) {
			flag = false;
		}
		return flag;
	...
	public static String waterwallRssTime(long time) {
		...
		long differ = 0;
		if (differTime > 0) {
			LogFactory.createLog().e("zjj, waterwallCTime:" + differTime + " min:" + (differTime / 60000));
			if ((differ = differTime / 60000) < 60) {
				differ = differ == 0 ? 1 : differ;
				return differ + "分钟前";
	...
	public static String getJsonString(String path) {
		...
		try {
			...
			if (!file.exists()) {
				LogFactory.createLog().i(path + "not contain the file !!!");
				return jsonStr;
			}
