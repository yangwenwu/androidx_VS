package com.lemon.video.fragment.video.activity
...
import com.lemon.video.fragment.video.MediaPlayer.JZExoPlayer
import com.lemon.video.fragment.video.adapter.CommentRecycleAdapter
import com.lemon.video.fragment.video.adapter.NewVideoRecycleAdapter2
import com.lemon.video.httpModel.CommentModel
import com.lemon.video.https.ResponseListener
import com.lemon.video.model.CommentBean
import com.lemon.video.model.HomeNewsBean
import com.lemon.video.model.ResObject
import com.lemon.video.model.UserBean
import com.lemon.video.utils.*
...
class VideoDetailActivity : BaseAppActivity(), View.OnClickListener {

    private var code: String? = null
    private var homeNewsBean: HomeNewsBean? = null

    private var reCommentAdapter: NewVideoRecycleAdapter2? = null
    ...
    private fun initView() {
        ...
        reCommentAdapter = NewVideoRecycleAdapter2(this, videoList)
...
//        video_title.
        ...
        if (homeNewsBean!!.bigTitleImage != null && homeNewsBean!!.bigTitleImage != "") {
            GlideApp.with(this)
                    .load(HttpConstants.SERVICEURL + StringUrlUtil.checkSeparator(homeNewsBean!!.bigTitleImage))
                    .centerCrop()
                    .dontAnimate()
                    .into(videoplayer.thumbImageView)
        } else {
            if (homeNewsBean!!.titleImage != null && homeNewsBean!!.titleImage != "") {
                GlideApp.with(this)
                        .load(HttpConstants.SERVICEURL + StringUrlUtil.checkSeparator(homeNewsBean!!.titleImage))
                        .centerCrop()
                        .dontAnimate()
                        .into(videoplayer.thumbImageView)
            }
...
//                        })
    ...
    //获取传过来的json地址
    private fun getData() {
        getVideoDetail(homeNewsBean!!.dataId) //视频详情
        getLatestData(1, code!!) //推荐列表
        if (BaseApp.isLogin) {
            //查询是否已经收藏
            ...
            queryCollectState(userId, homeNewsBean!!.dataId)
        }
//        getGoodCount()
    ...
    private fun queryCommentCount() {
        Commrequest.queryCommentCount(this@VideoDetailActivity, homeNewsBean!!.dataId, object : ResponseListener {
    ...
    private fun getGoodCount() {
        Commrequest.getGoodCount(this@VideoDetailActivity, homeNewsBean!!.dataId, object : ResponseListener {
            override fun onResponse(t: BaseJsonBean, code: Int) {
                val jsonObject = JSON.parseObject(t.`object`)
     ...
     * 评论列表
     */
    private fun queryCommentList(page: Int) {
        Commrequest.queryCommentList(this@VideoDetailActivity, homeNewsBean!!.dataId, page, object : ResponseListener {
            override fun onResponse(t: BaseJsonBean, code: Int) {
                val jsonObject = JSON.parseObject(t.`object`)
                val resMsg = jsonObject.getString("resMsg")
                if (resMsg == "success") {
    ...
    private fun cancelCollect() {
        ...
        val userBean = JSON.parseObject(userStr, UserBean::class.java)
        val userId = userBean.id
        Commrequest.cancelCollection(this@VideoDetailActivity, userId, homeNewsBean!!.dataId, object : ResponseListener {
            override fun onResponse(t: BaseJsonBean, code: Int) {
                //返回结果
                //{"resCode":"200","resMsg":"success","resObject":null}
    ...
    private fun addCollect() {
        ...
        val userBean = JSON.parseObject(userStr, UserBean::class.java)
        val userId = userBean.id
        Commrequest.addCollection(this@VideoDetailActivity, userId, homeNewsBean!!.dataId, object : ResponseListener {
            override fun onResponse(t: BaseJsonBean, code: Int) {
                //返回结果
                //{"resCode":"200","resMsg":"success","resObject":null}
    ...
    private val urlHandler = object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(msg: Message) {
            ...
            videoplayer.setUp(url, JZVideoPlayerStandard.SCREEN_WINDOW_NORMAL, homeNewsBean?.title)
            ...
            override fun onResponse(t: BaseJsonBean, code: Int) {
                ...
                if (jsonArray.size > 0) {
                    for (i in jsonArray.indices) {
                        ...
                        if (bean.dataId == homeNewsBean!!.dataId) {
                        } else {
                            videoList.add(bean)
                        }
                    }
...
//                    }
    ...
    private fun comment(content: String) {
        ...
        model.newsId = homeNewsBean!!.dataId
        model.type = "2"
    ...
    internal var itemClickListener = object : NewVideoRecycleAdapter2.OnItemClickListener {
